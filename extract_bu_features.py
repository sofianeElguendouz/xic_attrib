...
def create_bu_features(dataset):
    ...
    #print("on est avant CFG")
    cfg = get_cfg()
    cfg.merge_from_file("./external/buAtt/configs/VG-Detection/faster_rcnn_R_101_C4_attr_caffemaxpool.yaml")
    cfg.MODEL.RPN.POST_NMS_TOPK_TEST = 300
    cfg.MODEL.ROI_HEADS.NMS_THRESH_TEST = 0.6
    cfg.MODEL.ROI_HEADS.SCORE_THRESH_TEST = 0.2
    # VG Weight
    cfg.MODEL.WEIGHTS = "./external/buAtt/demo/faster_rcnn_from_caffe_attr.pkl"
    predictor = DefaultPredictor(cfg)
    #print("on est juste avant extract features !!!")
...
def extract_featrue(predictor, split, dataset_name, data, th=0.2):
    save_path = f'./dataset/{dataset_name}_bu_features/{split}'
    ...
    for idx,item in enumerate(data):
        #print(idx, len(data))
        img_path = "./dataset"+item['image_path'][1:]
...
def extract_single(raw_img_path, save_path, image_filename):
    ...
    with torch.no_grad():
        ...
        if roi_features.size(0) == 0:
            print("img with 0 roi : ", image_filename)
            return
...
if __name__ == '__main__':
    create_bu_features('flickr30k')
    #create_bu_features('coco2017')
