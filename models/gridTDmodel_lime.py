...
c = torch.load('mn_mx_bu_1024.pt')
g_mn_bu = c[0].unsqueeze(0)
g_mx_bu = c[1].unsqueeze(0)
...
#Flickr30k
c = torch.load('mn_mx_bu_1024_flickr30k.pt')
g_mn_bu = c[0].unsqueeze(0)
g_mx_bu = c[1].unsqueeze(0)
...
class GridTDModelBU(nn.Module):
    ...
    def beam_search(self, images_features,  word_map, perturb, pos, sd, beam_size=3,max_cap_length=20):
 
        #perturbIdx_outprob = [0]*37
        self.eval()
        ...
        with torch.no_grad():
            ...
            #print("top_k_scores ", top_k_scores.shape)
            seqs = torch.LongTensor([[word_map['<start>']]] *beam_size).cuda()  # (unfinished_num, )
            image_feature_proj = self.relu(self.img_projector(images_features)) # batch_size, 36, hidden_dim
            
            #----------------------------------------------------------------------------------
            if perturb == "VF" and pos!=None:
                if sd == "min":
                    for i in pos:
                        image_feature_proj[0][i] = torch.zeros(1, 1024)
                elif sd == "max":
                    for i in pos:
                        #print(image_feature_proj[0][i].shape)
                        image_feature_proj[0][i] = torch.full((1024,), 16.4)
                else:
                    for i in pos:
                        #perturbIdx_outprob[i] = 1
                        j = 0
                        while j < 1024:
                            sample = np.random.normal(0, sd)
                            if  g_mn_bu[0][j].item() == g_mx_bu[0][j].item():
                                j += 1
                            elif  (image_feature_proj[0][i][j] + sample >= g_mn_bu[0][j].item()) and (image_feature_proj[0][i][j] + sample <= g_mx_bu[0][j].item()):
                                image_feature_proj[0][i][j] += sample
                                j += 1
            ...
            #print(global_img_feature.shape)
            ...
            for step in range(max_cap_length):
                ...
                #----------------------------------------------------------------------------------
                if perturb == "WE" and (pos == -1 or step == pos):
                    #print("Perturbation of WE !")
                    for i in range(word_embedding.shape[0]):
                        j = 0
                        while j < 512:
                            sample = np.random.normal(0, sd)
                            #print("Feature : {} | Perturb : {} | Min : {} | Max : {}".format(word_embedding[i][j], sample, mn_we, mx_we))
                            if (word_embedding[i][j] + sample >= mn_we) and (word_embedding[i][j] + sample <= mx_we):
                                word_embedding[i][j] += sample
                                j += 1
                    """
                    if sd == "max":
                        sample = np.random.binomial(1, .5, 512)
                        for i in range(word_embedding.shape[0]):
                            j = 0
                            while j < 512:
                                if sample[j] == 0:
                                    word_embedding[i][j] = mn_we
                                else:
                                    word_embedding[i][j] = mx_we
                    else:
                    """
                ...
                h1t, c1t, h2t, c2t = state  # (bs, hidden_dim, )
                h1t, c1t, st = self.AdaLSTM(xt, (h1t, c1t))  #(bs, hidden_dim, )
                
                #----------------------------------------------------------------------------------
                if perturb == "HT" and (pos == -1 or step == pos) :
                    for i in range(h1t.shape[0]):
                        j = 0
                        while j < 1024:
                            sample = np.random.normal(0, sd)
                            if (h1t[i][j] + sample >= mn_ht) and (h1t[i][j] + sample <= mx_ht):
                                h1t[i][j] += sample
                                j += 1
                ...
                #----------------------------------------------------------------------------------
                if perturb == "CT" and (pos == -1 or step == pos) :
                    for i in range(context_t_hat.shape[0]):
                        j = 0
                        while j < 512:
                            sample = np.random.normal(0, sd)
                            if (context_t_hat[i][j] + sample >= mn_ct) and (context_t_hat[i][j] + sample <= mx_ct):
                                context_t_hat[i][j] += sample
                                j += 1
                ...
                #print("motorcycle: ", predict_score_t[:, 208])
                themaxidx = torch.argmax(predict_score_t).item()%vocab_size
                #print("max score, indx, word: ", torch.max(predict_score_t).item(), themaxidx, rev_word_map[themaxidx])
                
                top_k_scores_exp = top_k_scores.expand((unfinished_num, vocab_size))
                #print("top_k_scores_exp : ", top_k_scores_exp.shape)
                scores = top_k_scores_exp + predict_score_t
                #print(scores.shape)
                #print("predict score t : ", predict_score_t[:, 208])

                probs.append(torch.max(predict_score_t, 0).values)
                #print("probs : ", predict_score_t)
                #print("Max probs : ", torch.max(predict_score_t, 0).values)

                #themaxidx = torch.argmax(scores).item()%vocab_size
                #print("max score, indx, word: ", torch.max(scores).item(), themaxidx, rev_word_map[themaxidx])
                ...
                beam_idx = top_words / vocab_size  # (unfinished_num, )
                next_word_idx = top_words % vocab_size  # (unfinished_num, )
                #print("word : ", rev_word_map[208])
                #print('next_word_idx',next_word_idx)
            ...
            #perturbIdx_outprob[36] = max(target_word_probs).item()
            probs = torch.stack(probs)
            outprobs = torch.max(probs, 0).values
            #print(outprobs)
            return sentence, sen_idx, outprobs
